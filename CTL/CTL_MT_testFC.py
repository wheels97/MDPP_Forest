...
class CausalTree:
    def __init__(self, cont=False, max_depth=-1, min_size=2, weight=0.5, seed=None, split_size=0.5, honest=False,
        ...
        self.quartile = quartile#trigger选取四分位点
        self.quantile = quantile#特征分裂 quantile sketch
        ...
        self.start = 0.0
        self.time = 0.0
        self.max_values = max_values #分裂时X的取值范围
        self.mse = 0.0
        self.tnums = tnums
    ...
    def fit(self, rows, labels, treatment):
        ...
        if self.eval_func == self.objective:
            if self.val_honest or self.honest:
                ...
                _, effect = tau_squared(est_labels, est_treatment)#返回ACE
                p_val = get_pval(est_labels, est_treatment)#返回p_value
                ...
                current_var = (1 + train_to_est_ratio) * (
                        (current_var_treat / num_treat) + (current_var_control / num_cont))#Honest项
            else:
                _, effect = tau_squared(labels, treatment)
                p_val = get_pval(labels, treatment)

        elif self.eval_func == self.objective_cont:
        #处理continues t，输出叶子的trigger、effect、p_value、当前F
            if self.val_honest or self.honest:
                if self.val_honest:
                    train_rows, est_rows, train_outcome, est_labels, train_treat, est_treatment = \
                        train_test_split(rows, labels, treatment, shuffle=True, test_size=self.split_size)
        ...
        for col in range(0, column_count):
            ...
            if self.quantile:
                if (isinstance(unique_vals[0], int) or isinstance(unique_vals[0], float)):
                    if len(unique_vals) > 7:  # 找到一组数的分位数值
                        lspercentile = np.percentile(column_values, [0,10,25,40,55,70,90])
                    else:
                        lspercentile = np.percentile(unique_vals, [25,75])
...
#                 del treatment
    ...
    def objective_cont(self, train_outcome, train_treatment, val_outcome, val_treatment):
        ...
        if total_train == 0 or t_nums == 0 or total_val == 0:
            return return_val
        ...
        for trt in list(np.random.choice(a=range(t_nums), size=t_nums, replace=False, p=pp.ravel())):
            ...
            var_treatment_i = val_treatment[:,trt]
    
            range_treatment = np.array(list(filter(lambda x:x<=1-resource_used, train_treatment[:,trt])))
            #range_treatment_var = np.array(list(filter(lambda x:x<=1-resource_used, var_treatment[:,trt])))
...
#                 print(step,resource_used,trt,unique_treatment,max(range_treatment))
            ...
            if len(unique_treatment) > 9:  # 找到一组数的分位数值（需要降低个数，以免取到极端值）
...
#                 train_outcome = string_to_int(train_outcome)
            ...
            yv = np.tile(val_outcome, (unique_treatment.shape[0]+1, 1)) ###
            tv = np.tile(var_treatment_i, (unique_treatment.shape[0]+1, 1))

            ti = np.transpose((np.transpose(tt) >= train_interval[:-1])  & (np.transpose(tt) < train_interval[1:]) )
            tiv = np.transpose((np.transpose(tv) >= train_interval[:-1])  & (np.transpose(tv) < train_interval[1:]) )###
            
            # do the min_size check on each_interval
            interval_num = np.sum(ti == 1, axis=1)
            interval_numv = np.sum(tiv == 1, axis=1)###
#             if np.min(interval_num)==0:
#                 print('np.min(interval_num)==0',train_treatment_i,train_interval)
            min_interval_size = np.where((interval_num >= 1)&(interval_numv >= 1))
            min_interval_sizev = np.where((interval_numv >= 1) & (interval_num >= 1))
            ti = ti[ min_interval_size]
            tt = tt[ min_interval_size]
            yy = yy[ min_interval_size]
            tiv = tiv[ min_interval_sizev]###
            tv = tv[ min_interval_sizev]
            yv = yv[ min_interval_sizev]
            ...
            var_outcome_ii = np.sum((yv * (tiv == 1)), axis=1) / np.sum(tiv == 1, axis=1)     ###       
            var_treatment_ii = ((train_interval[:-1]+train_interval[1:])/2 )[ min_interval_sizev]
            #######################
            ...
            yyv = np.tile(var_outcome_ii, (unique_treatment.shape[0], 1)) ###
            ttv = np.tile(var_treatment_ii, (unique_treatment.shape[0], 1))
            wallv = np.tile(train_interval, (unique_treatment.shape[0], 1)) 
            yyv = np.cumsum((wallv[:,1:]-wallv[:,:-1])[:,min_interval_sizev].reshape(yyv.shape)*yyv,axis=1)
            ...
            #obj = train_err
            #obj = total_train * train_err #原始目标
            ###FC
